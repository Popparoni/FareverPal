...
//! lookup. There is NO write path: the process handle is opened read-only and
//! the reader exposes no write/alloc/protect — the companion only ever reads.
//!
...
impl Reader {
    #[staticmethod]
    fn attach(name: &str) -> PyResult<Reader> {
        let pid = find_pid(name)
            .ok_or_else(|| PyOSError::new_err(format!("process '{name}' not found")))?;
        // Read-only access only: we request just VM-read + query rights and
        // never the VM-write / VM-operation rights, so the OS handle itself
        // cannot be used to write to the game.
